...
import { parseBindMounts } from "./sandbox-paths.js";
...
export function createOpenClawCodingTools(options?: {
  ...
  const sandboxBindMounts = sandbox ? parseBindMounts(sandbox.docker.binds) : undefined;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      if (sandboxRoot) {
        ...
        return [
          workspaceOnly
            ? wrapToolWorkspaceRootGuard(sandboxed, sandboxRoot, sandboxBindMounts)
            : sandboxed,
        ];
      }
